import Mp3LameEncoder from "./lib/Mp3LameEncoder.min.js";

export default class mp3cutter {
  //libPath must end with a slash
  constructor(libPath = "./lib/", log = false) {
    window.self.Mp3LameEncoderConfig = {
  ...
  async cut(src, start, end, callback, bitrate = 192) {
    ...
    this.audioContext
      ...
      .then((decodedData) => this.computeData(decodedData));
  }
  ...
  computeData(decodedData) {
    ...
    let formattedArray = {
      channels: Array.apply(null, {
        length: newBuffer.numberOfChannels - 1 - 0 + 1
      })
        .map((v, i) => i + 0)
